...
import { Calendar, Loader2, Plus, CheckCircle2, Lock, Unlock, Trash2, Circle } from "lucide-react";
          ...
          {rows.map((p) => (
            <div key={p.id} className={`bg-white rounded-2xl border p-5 transition-all ${p.is_active ? "border-teal-400 ring-2 ring-teal-100" : "border-slate-200"}`} data-testid={`period-card-${p.kode}`}>
              ...
              <div className="flex items-center gap-2 mt-4">
                {!p.is_active && <Button size="sm" variant="outline" className="rounded-lg text-xs" onClick={() => activate(p.id)} data-testid={`activate-${p.kode}`}>Aktifkan</Button>}
                ...
                {!p.is_active && <Button size="sm" variant="ghost" className="rounded-lg text-xs text-red-500 hover:text-red-600 ml-auto" onClick={() => del(p.id)} data-testid={`del-${p.kode}`}><Trash2 className="w-3.5 h-3.5" /></Button>}
              </div>
            </div>
          ))}
        </div>
